			 ...
			 struct rcu_data *rdp)
{
	...
	if (rcu_gp_in_progress(rsp) ||
	    !need_any_future_gp(rcu_get_root(rsp)) ||
	    time_before(jiffies, READ_ONCE(rsp->gp_req_activity) + HZ) ||
	    time_before(jiffies, READ_ONCE(rsp->gp_activity) + HZ) ||
	    atomic_read(&warned))
		return;
	...
	if (rcu_gp_in_progress(rsp) ||
	    !need_any_future_gp(rcu_get_root(rsp)) ||
	    time_before(jiffies, rsp->gp_req_activity + HZ) ||
	    time_before(jiffies, rsp->gp_activity + HZ) ||
	    atomic_xchg(&warned, 1)) {
		raw_spin_unlock_irqrestore_rcu_node(rnp_root, flags);
		raw_spin_unlock_irqrestore_rcu_node(rnp, flags);
		return;
	}
	pr_alert("%s: g%lu %d%d%d%d gar:%lu ga:%lu f%#x\n",
		 ...
		 jiffies - rsp->gp_req_activity,
		 jiffies - rsp->gp_activity,
		 rsp->gp_flags);
	WARN_ON(1);
	raw_spin_unlock_irqrestore_rcu_node(rnp_root, flags);
	raw_spin_unlock_irqrestore_rcu_node(rnp, flags);
